...
import { ProgressBar } from "@/components/career/ui";
import { api } from "@/lib/api";
import { gameCatalog, getGameMaxLevel } from "@/lib/career-data";
...
export function GameRunner({ gameId, level = 1, mazeSeed }: Props) {
  ...
  const addSession = useCareerStore((state) => state.addSession);
  const addAction = useCareerStore((state) => state.addAction);
